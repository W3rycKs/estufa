...
function renderizartelademonitoramento(codigo) {
		...
		var new_header_p_text = document.createTextNode("Conectado a "+codigo) 
		header_p.innerHTML = ""
    	...
    	if(xhr.readyState == 4) {
            ...
            result.forEach(function array(obj) {
				if(code==obj.code) {
					...
					++value;
					if (antiloop==0) {renderizartelademonitoramento(obj.code); ++antiloop}
				}
				else {if (antiloop==0) {alert("codigo não encontrado, tente novamente"); ++antiloop}
				}
})
...
function temper() {
	...
	xhr.onreadystatechange = () => {
    	if(xhr.readyState == 4) {
        	if(xhr.status == 200) {
            var result = JSON.parse(xhr.responseText);
            if (result[0]!=null) {temperatura.innerHTML = result[0].temp.toFixed(2) + "°C"}
            else {temperatura.innerHTML = result[1].temp.toFixed(2) + "°C"}	
            //console.log(result)
        }
    }
};
...
function umida() {
	...
	xhr.onreadystatechange = () => {
    	if(xhr.readyState == 4) {
        	if(xhr.status == 200) {
            var result = JSON.parse(xhr.responseText);
            if (result[0]!=null) {umidade.innerHTML = result[0].umidade_ar.toFixed(2) + "%"}
            else {umidade.innerHTML = result[1].umidade_ar.toFixed(2) + "%"}	
            //console.log(result)
        }
    }
};
...
function umidasolo() {
	...
	xhr.onreadystatechange = () => {
    	if(xhr.readyState == 4) {
        	if(xhr.status == 200) {
            var result = JSON.parse(xhr.responseText);
            if (result[0]!=null) {umidade_solo.innerHTML = result[0].umidade_solos.toFixed(2) + "%"}
            else {umidade_solo.innerHTML = result[1].umidade_solo.toFixed(2) + "%"}	
            //console.log(result)
        }
    }
};
