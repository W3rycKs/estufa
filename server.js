...
var client = mqtt.connect(process.env.MQTT_BROKER_CON)//('mqtt://broker.hivemq.com')//('mqtt://broker.mqtt-dashboard.com') ->
...
//BANCO_DE_DADOS-----------------------------------------------------------------------
function iniciar_banco(obj) {
mongoose.connect(process.env.MONGO_URL_CON) 
...
const dados = Model(obj.code,esp8266_schema) //-----> nome do banco
data = dados;
}
...
function find_bank(obj) {
	data.find((err,document)=> {
		...
		array_json = document;
		if (array_json.length<20) {json_for_send = array_json}
		else {
		for (var n1=array_json.length; n1>=array_json.length-20; --n1) {json_for_send.push(array_json[n1])}//>ultimas 20 mensagens
		if (json_for_send.length>=40) {for (var n1=0; n1<=20; ++n1) {json_for_send.shift()}}	
