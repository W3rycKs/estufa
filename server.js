...
function find_bank(obj) {
	data.find((err,document)=> {
		...
		array_json = document;
		if (array_json.length<150) {json_for_send = array_json}
		else {
		for (var n1=array_json.length; n1>=array_json.length-150; --n1) {json_for_send.push(array_json[n1])}//>ultimas 20 mensagens
		if (json_for_send.length>=300) {for (var n1=0; n1<=150; ++n1) {json_for_send.shift()}}	
